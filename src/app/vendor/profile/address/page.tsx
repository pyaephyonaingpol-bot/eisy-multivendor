...
export default async function VendorProfileAddressPage() {
  const session = await getSessionProfile();
  if (!session) redirect("/login?next=/vendor/profile/address");
    ...
    <div className="space-y-6">
      <div className="space-y-2">
        ...
        <p className="max-w-xl text-sm text-zinc-600">
          Registered business address used for KYC and payout records.
        </p>
      </div>
      <VendorProfileFieldForm vendor={vendor} field="address" />
    </div>
  );
}
