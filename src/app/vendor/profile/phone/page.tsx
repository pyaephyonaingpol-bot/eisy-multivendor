...
export default async function VendorProfilePhonePage() {
  const session = await getSessionProfile();
  if (!session) redirect("/login?next=/vendor/profile/phone");
    ...
    <div className="space-y-6">
      <div className="space-y-2">
        ...
        <p className="max-w-xl text-sm text-zinc-600">
          Phone number for ops contact and verification follow-ups.
        </p>
      </div>
      <VendorProfileFieldForm vendor={vendor} field="phone" />
    </div>
  );
}
