...
/**
 * Polls the deposit address and matches unused transfers to pending intents
 * by amount (shared deposit address — no TRC-20 memo).
 */
export async function monitorPendingUsdtDeposits(options?: {
  ...
  if (!depositAddress) {
    return {
      ...
      confirmed: 0,
      skipped: 0,
      errors: ["USDT_TRC20_DEPOSIT_ADDRESS is not configured."],
      results: [],
    };
  }
  ...
  const transfers = await listRecentUsdtTrc20Transfers({
  ...
  const { data: intentRows, error: intentError } = await supabase
    ...
    .select(
      "id, amount_usdt, deposit_address, status, expires_at, order_ids, tx_hash",
    )
  ...
  // Prefer unique exact-amount intents first to reduce collisions.
  const remaining = [...openIntents];
  ...
  for (const transfer of transfers) {
    ...
    const candidates = remaining.filter((intent) =>
    ...
    // Exact amount preferred; otherwise earliest pending intent.
    candidates.sort((a, b) => {
      const aExact =
        Math.abs(Number(a.amount_usdt) - transfer.amountUsdt) < 0.000001 ? 0 : 1;
      const bExact =
        Math.abs(Number(b.amount_usdt) - transfer.amountUsdt) < 0.000001 ? 0 : 1;
    ...
    try {
      ...
      // Re-verify via the shared helper (amount window + destination).
      const verified = await verifyUsdtTrc20Transfer({
        txHash: transfer.txHash,
        expectedToAddress: intent.deposit_address || depositAddress,
      ...
      // Optional sweep enqueue after confirmed checkout.
      try {
        ...
        // Sweep is optional; settings may disable it.
      }
    } catch (error) {
      errors.push(
        `${intent.id}: ${error instanceof Error ? error.message : "match failed"}`,
      );
    }
  }
