/**
 * ISO 3166-1 alpha-2 countries and common shipping territories.
 * Used by checkout, profile, and destination selectors.
 * Myanmar (MM) is pinned first for Eisy Myanmar buyers; remaining entries are A–Z by label.
 */

export type CountryOption = { code: string; label: string };

const GLOBAL_COUNTRIES_ALPHA: CountryOption[] = [
...
/** Myanmar first (marketplace default), then every other country A–Z. */
export const ALL_COUNTRY_OPTIONS: CountryOption[] = (() => {
  const myanmar = GLOBAL_COUNTRIES_ALPHA.find((c) => c.code === "MM");
  const rest = GLOBAL_COUNTRIES_ALPHA.filter((c) => c.code !== "MM").sort((a, b) =>
    a.label.localeCompare(b.label, "en"),
  );
  return myanmar ? [myanmar, ...rest] : rest;
})();
