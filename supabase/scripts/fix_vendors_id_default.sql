-- =============================================================================
-- 038_fix_vendors_id_default.sql
--
-- Live public.vendors often exists without DEFAULT on id (CREATE TABLE IF NOT
-- EXISTS is a no-op). apply_for_vendor then inserts without id and fails:
--   null value in column "id" of relation "vendors" violates not-null constraint
-- =============================================================================

create extension if not exists "pgcrypto";

-- Ensure id always auto-generates when omitted from INSERT
do $$
begin
  if exists (
    select 1
...
-- Backfill any accidental null ids (should be none; primary key usually blocks)
update public.vendors
set id = gen_random_uuid()
where id is null;

-- Recreate apply_for_vendor with an explicit UUID so inserts work even if
-- the column default is still missing for any reason.
